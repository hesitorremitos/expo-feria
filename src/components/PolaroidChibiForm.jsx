...
const PolaroidChibiForm = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			console.log('🚀 Enviando petición a /api/generate-polaroid-chibi...');
			const response = await fetch('/api/generate-polaroid-chibi', {
				method: 'POST',
				body: submitData
			});
	...
	const handleCloseResult = useCallback(() => {
		console.log('🔴 PolaroidChibiForm: Closing result modal');
		setResult(null);
	}, []);
			...
			<form onSubmit={handleSubmit} className="space-y-8">
				...
				<div className="space-y-6">
					...
					<div className="max-w-sm mx-auto">
						...
						<p className="text-xs text-accent-500 mt-2 text-center">
							Tu foto se convertirá en un chibi 3D saliendo de una Polaroid 📷
						</p>
					</div>
				</div>
				...
				<div className="space-y-4">
					...
					<div className="max-w-lg mx-auto">
						<textarea
							...
							onChange={handleInputChange}
							rows="3"
							placeholder="Ej: Expresión alegre, saltando de la foto, rodeado de confeti, con gafas..."
						...
						/>
						<p className="text-xs text-accent-500 mt-2 text-center">
							Describe pose, expresión o elementos creativos para tu chibi saliendo de Polaroid
						</p>
					</div>
				</div>
				...
				<div className="text-center space-y-4">
					<Button
						...
						className="bg-gradient-to-r from-pink-600 to-purple-700 hover:from-pink-700 hover:to-purple-800 text-white px-8 py-4 text-lg font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 disabled:opacity-50 disabled:transform-none disabled:shadow-lg"
					>
						{isGenerating ? 'Saliendo de la foto...' : '📷 Crear Polaroid Chibi'}
					</Button>
				...
				{/* Información sobre Polaroid Chibi */}
				<div className="bg-pink-50 rounded-xl p-6 border border-pink-200 max-w-2xl mx-auto">
					<div className="text-center text-sm text-primary-700 space-y-2">
						<p className="font-semibold text-base">📷 ¿Qué es Polaroid Chibi?</p>
						<p>Un chibi 3D adorable literalmente saliendo de una foto Polaroid vintage con efecto de profundidad. El personaje rompe la 4ta pared emergiendo del marco, creando una composición única y creativa perfecta para redes sociales 💜</p>
						<p className="text-xs text-accent-600">Incluye render 3D profesional y estética Polaroid auténtica</p>
					</div>
				</div>
...
export default PolaroidChibiForm;
