...
import { generateGhibliStyle } from '../../lib/ghibliStyleGenerator.js';
...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		const result = await generateGhibliStyle({
		...
		console.error('Error in generate-ghibli-style API:', error);
		return new Response(
			JSON.stringify({ 
				success: false, 
