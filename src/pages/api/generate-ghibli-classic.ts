...
import { generateGhibliClassic } from '../../lib/ghibliClassicGenerator.js';
...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		const result = await generateGhibliClassic({
		...
		console.error('Error in generate-ghibli-classic API:', error);
		return new Response(
			JSON.stringify({ 
				success: false, 
